...
User.create = (user) => {
  ...
  const sql = /*sql*/ `INSERT INTO users(email, name, lastname, phone, image, password, created_at, updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`;
...
User.update = (user) => {
  const sql = /*sql*/  `UPDATE users SET name = $2, lastname = $3, phone = $4, image = $5, updated_at = $6 WHERE id = $1`;

  return db.none(sql, [user.id, user.name, user.lastname, user.phone, user.image, new Date()]);
}

User.updateToken = (id,token) => {
  const sql = /*sql*/  `UPDATE users SET session_token = $2  WHERE id = $1`;
